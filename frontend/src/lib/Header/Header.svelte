<script lang="ts">
    ...
    const logout = () => {
        execute('logout', 'POST').then((_) => (window.location.href = 'http://74.234.48.140:8080'));
    };
...
<header class="flex justify-between bg-grass p-2 h-16 align-middle">
    ...
    <div class="hidden lg:block lg:flex lg:flex-row lg:w-full lg:justify-center lg:items-center">
        <div
            ...
            on:click={() => (window.location.href = 'http://74.234.48.140:8080/announcements')}
            on:keydown={() => (window.location.href = 'http://74.234.48.140:8080/announcements')}
        >
            <span class={pageType === 'announcements' ? 'text-ivory' : 'text-cocoa'}> Ogłoszenia </span>
        </div>
        <div
            class="hidden lg:block group hover:cursor-pointer text-taupe px-4 text-xl"
            on:click={() => (window.location.href = 'http://74.234.48.140:8080/meetings')}
            on:keydown={() => (window.location.href = 'http://74.234.48.140:8080/meetings')}
        >
            <span class={pageType === 'meetings' ? 'text-ivory' : 'text-cocoa'}>Spotkania</span>
        </div>
        <div
            class="hidden lg:block group hover:cursor-pointer text-taupe px-4 text-xl"
            on:click={() => (window.location.href = 'http://74.234.48.140:8080/events')}
            on:keydown={() => (window.location.href = 'http://74.234.48.140:8080/events')}
        >
            <span class={pageType === 'events' ? 'text-ivory' : 'text-cocoa'}>Wydarzenia</span>
        </div>
        <div
            class="hidden lg:block group hover:cursor-pointer text-taupe px-4 text-xl"
            on:click={() => (window.location.href = 'http://74.234.48.140:8080/map')}
            on:keydown={() => (window.location.href = 'http://74.234.48.140:8080/map')}
        >
